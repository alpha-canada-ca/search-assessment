...
public class Airtable implements DataTable {
    ...
    public List<Department> getDepartments() {
        try {
            ...
            ClassicHttpResponse httpResponse = (ClassicHttpResponse) httpClient.execute(get);
            ...
            LOG.error("Could not parse the response from Airtable.", e);
        }
        return null;
    }
            ...
            if (httpResponse.getCode() == HttpStatus.SC_OK) {
                ...
                if (!results.isEmpty() && results.has(RESULTS_SET) && !results.getJSONArray(RESULTS_SET).isEmpty()) {
                    ...
                    if (type != SearchType.CONTEXTUAL) {
                        ...
                        httpResponse = (ClassicHttpResponse) httpClient.execute(get);
                        if (httpResponse.getCode() == HttpStatus.SC_OK) {
